...
from .data_utils import block_orthogonal, get_dropout_mask
...
class LstmCellWithProjection(torch.nn.Module):
    ...
    def reset_parameters(self):
        ...
        self.state_linearity.bias.data[self.cell_size:2 *
                                       self.cell_size].fill_(1.0)
    ...
    def forward(
        ...
        output_accumulator = Variable(
            inputs.data.new(batch_size, total_timesteps,
                            self.hidden_size).fill_(0))
        if initial_state is None:
            full_batch_previous_memory = Variable(
                inputs.data.new(batch_size, self.cell_size).fill_(0))
            full_batch_previous_state = Variable(
                inputs.data.new(batch_size, self.hidden_size).fill_(0))
        else:
        ...
        for timestep in range(total_timesteps):
            ...
            full_batch_previous_state[0:current_length_index +
                                      1] = timestep_output
            output_accumulator[0:current_length_index +
                               1, index] = timestep_output
