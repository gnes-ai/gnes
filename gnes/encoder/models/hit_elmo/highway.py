...
class Highway(nn.Module):
    ...
    def __init__(self,
                 input_dim: int,
                 num_layers: int = 1,
                 activation: Callable[[torch.Tensor], torch.Tensor] = torch.nn.
                 functional.relu) -> None:
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:    # pylint: disable=arguments-differ
        ...
        for layer in self._layers:
            ...
            # above, too.
            nonlinear_part = projected_input[:, (0 * self._input_dim):(
                1 * self._input_dim)]
            gate = projected_input[:, (1 * self._input_dim):(
                2 * self._input_dim)]
