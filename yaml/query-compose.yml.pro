...
services:
  ...
  outgoing-proxy:
    image: ${DOCKER_IMG_URL}
    command: >
      proxy --proxy_type=ReduceProxyService --port_in ${OUTGOING_PROXY_IN} --port_out ${OUTGOING_PROXY_OUT}
      --socket_in SUB_BIND --socket_out PUB_BIND --num_part=$NUM_REP
      --unk_msg_route DEFAULT
    networks:
      gnes-net:
        aliases:
  ...
  encoder:
    ...
    command: >
      ...
      --mode QUERY --yaml_path /test_encoder.yml --dump_path /dump/model.bin
    volumes:
  ...
  indexer:
    ...
    command: >
      index --port_in ${MIDDLEMAN_PROXY_OUT} --port_out ${OUTGOING_PROXY_IN}
       ...
       --mode QUERY --yaml_path /indexer.yml --dump_path /out_data/test_index.bin
    volumes:
